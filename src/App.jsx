...
const App = () => {
  ...
  // ── hero images — each with custom framing ──
  const heroSlides = [
    { src: BASE + 'images/DSC08584.jpg', pos: 'center 40%' },
    { src: BASE + 'images/DSC08619.jpg', pos: 'center 35%' },
    { src: BASE + 'images/DSC08641.jpg', pos: 'center 45%' },
    { src: BASE + 'images/DSC08671.jpg', pos: 'center 30%' },
    { src: BASE + 'images/DSC08800.jpg', pos: 'center 38%' },
  ];
      ...
      <nav style={{display:'flex',gap:'1.8rem',alignItems:'center'}}>
        {['About','Schedule','Gallery','Join'].map(s=>(
          <a key={s} href={`#${s.toLowerCase()}`} style={{
            color:'rgba(240,239,239,.75)',textDecoration:'none',fontSize:'13px',fontWeight:500,
            letterSpacing:'.5px',textTransform:'uppercase',transition:`color .3s ${ease}`,
          }}
            onMouseEnter={e=>{e.target.style.color=c.pink}}
            onMouseLeave={e=>{e.target.style.color='rgba(240,239,239,.75)'}}
          >{s}</a>
        ))}
      </nav>
    </header>
  ...
  // ═══════════════════════════════════════════
  //  HERO — cinematic 16:9-ish, images properly framed
  // ═══════════════════════════════════════════
  const Hero = () => (
    <section style={{position:'relative',height:'88vh',maxHeight:'900px',overflow:'hidden'}}>
      ...
      {/* gradient — heavier at bottom, lighter everywhere else so you see the image */}
      <div style={{position:'absolute',inset:0,zIndex:2,
        background:'linear-gradient(180deg, rgba(10,10,20,.3) 0%, rgba(10,10,20,.08) 30%, rgba(10,10,20,.15) 55%, rgba(10,10,20,.85) 85%, rgba(10,10,20,1) 100%)',
      }}/>

      {/* content pinned bottom-left */}
      <div style={{
        position:'absolute',bottom:0,left:0,right:0,zIndex:3,
        padding:'0 clamp(2rem,6vw,5rem) clamp(3rem,8vh,5rem)',
        maxWidth:'1400px',
      }}>
        <p style={{
          fontSize:'12px',fontWeight:600,color:c.pink,textTransform:'uppercase',letterSpacing:'4px',
          marginBottom:'14px',animation:`up .7s ${ease} .2s both`,
        }}>Clifton, NJ &mdash; Indoor Turf</p>

        <h1 style={{
          fontWeight:900,lineHeight:.9,letterSpacing:'-.04em',marginBottom:'.35em',
          animation:`up .7s ${ease} .3s both`,
        }}>
          <span style={{display:'block',fontSize:'clamp(52px,11vw,140px)'}}>7 AM.</span>
          <span style={{display:'block',fontSize:'clamp(36px,7vw,95px)',fontWeight:300,
            letterSpacing:'-.02em',color:'rgba(240,239,239,.8)',
          }}>No Excuses.</span>
        </h1>

        <p style={{
          fontSize:'clamp(15px,1.6vw,20px)',fontWeight:300,color:'rgba(240,239,239,.65)',
        ...
        <div style={{display:'flex',gap:'14px',flexWrap:'wrap',animation:`up .7s ${ease} .6s both`}}>
          <a href="https://opensports.net/21fc" target="_blank" rel="noopener noreferrer"
            style={{
              padding:'15px 40px',background:c.pink,color:'#fff',fontSize:'12px',fontWeight:700,
          ...
          <a href="#schedule"
            style={{
              padding:'15px 28px',background:'transparent',color:'rgba(240,239,239,.7)',
      ...
      {/* scroll indicator */}
      <div style={{position:'absolute',bottom:'1.5rem',left:'50%',transform:'translateX(-50%)',zIndex:3,opacity:.5}}>
        <div style={{width:'20px',height:'34px',border:'1px solid rgba(255,255,255,.25)',borderRadius:'10px',display:'flex',justifyContent:'center',paddingTop:'6px'}}>
  ...
  // ═══════════════════════════════════════════
  //  ABOUT — split layout, image properly contained
  // ═══════════════════════════════════════════
  const About = () => (
    <section id="about" data-r style={{
      display:'grid',gridTemplateColumns:'1fr 1fr',minHeight:'550px',
      ...
      <div style={{overflow:'hidden',position:'relative',...reveal('about',0)}}>
        ...
        <div style={{position:'absolute',bottom:0,left:0,right:0,height:'40%',
          background:'linear-gradient(transparent,rgba(10,10,20,.5))'}}/>
      </div>

      <div style={{background:c.card,padding:'clamp(2.5rem,6vw,5rem)',display:'flex',flexDirection:'column',justifyContent:'center',
        ...
        <h2 style={{fontSize:'clamp(26px,3.5vw,44px)',fontWeight:300,lineHeight:1.2,marginBottom:'1.5rem'}}>
          Unite skilled players<br/>in real competition
        </h2>
        <p style={{fontSize:'clamp(15px,1.5vw,17px)',fontWeight:300,lineHeight:1.8,color:'rgba(240,239,239,.75)',marginBottom:'2rem'}}>
  ...
  // ═══════════════════════════════════════════
  const Stats = () => (
    <section id="stats" data-r style={{
      background:c.bg,borderTop:'1px solid rgba(255,255,255,.04)',borderBottom:'1px solid rgba(255,255,255,.04)',
      padding:'2.5rem 2rem',
    }}>
      <div style={{maxWidth:'1000px',margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem',textAlign:'center'}}>
        {[{v:'200+',l:'Players'},{v:'4×',l:'Weekly'},{v:'7 AM',l:'Kickoff'},{v:'3+',l:'Years'}].map((s,i)=>(
          <div key={i} style={reveal('stats',i*.08)}>
            <div style={{fontSize:'clamp(24px,4vw,40px)',fontWeight:800,color:c.white,lineHeight:1}}>{s.v}</div>
  ...
  // ═══════════════════════════════════════════
  //  IMAGE BAND — 3 photos side by side, properly framed
  // ═══════════════════════════════════════════
  const ImageBand = ({ images, height = '280px' }) => (
    <div style={{display:'grid',gridTemplateColumns:`repeat(${images.length},1fr)`,gap:'3px',background:c.black}}>
      {images.map((img,i)=>(
        <div key={i} style={{height,overflow:'hidden'}}>
          <img src={img.src} alt="" loading="lazy"
            style={{
              width:'100%',height:'100%',objectFit:'cover',
              objectPosition: img.pos || 'center 35%',
              filter:'brightness(.88)',
              transition:`transform .8s ${ease}, filter .5s ${ease}`,
            }}
            onMouseEnter={e=>{e.target.style.transform='scale(1.04)';e.target.style.filter='brightness(1)';}}
  ...
  // ═══════════════════════════════════════════
  const Schedule = () => (
    <section id="schedule" data-r style={{background:c.bg,padding:'clamp(4rem,8vh,7rem) 2rem'}}>
      <div style={{maxWidth:'1100px',margin:'0 auto'}}>
        <div style={{marginBottom:'2.5rem',...reveal('schedule')}}>
          ...
          <h2 style={{fontSize:'clamp(28px,4.5vw,48px)',fontWeight:600,marginBottom:'.4rem'}}>Weekly Schedule</h2>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px'}}>
          {['Monday','Wednesday','Friday','Sunday'].map((day,i)=>(
            ...
            >
              ...
              <div style={{fontSize:'26px',fontWeight:700,marginBottom:'.3rem'}}>7:00 AM</div>
  ...
  // ═══════════════════════════════════════════
  const Parallax = ({ src, pos = 'center 35%', h = '40vh', overlay = .45, children }) => (
    <div style={{position:'relative',height:h,overflow:'hidden'}}>
      <div style={{
        ...
        backgroundPosition:pos,backgroundAttachment:'fixed',
      }}/>
  ...
  // ═══════════════════════════════════════════
  //  GALLERY — masonry-style grid, every image carefully positioned
  // ═══════════════════════════════════════════
  const Gallery = () => {
    const imgs = [
      { src: BASE+'images/DSC08823.jpg', col:'1/3', row:'1/3', pos:'center 25%' },
      { src: BASE+'images/DSC08584.jpg', col:'3/4', row:'1/2', pos:'center 35%' },
      { src: BASE+'images/DSC08634.jpg', col:'4/5', row:'1/2', pos:'center 40%' },
      { src: BASE+'images/DSC08703.jpg', col:'3/4', row:'2/3', pos:'center 35%' },
      { src: BASE+'images/DSC08806.jpg', col:'4/5', row:'2/3', pos:'center 40%' },
      { src: BASE+'images/DSC08730.jpg', col:'1/2', row:'3/4', pos:'center 30%' },
      { src: BASE+'images/DSC08820.jpg', col:'2/3', row:'3/4', pos:'center 35%' },
      { src: BASE+'images/DSC08712.jpg', col:'3/5', row:'3/4', pos:'center 30%' },
    ];
    return (
      <section id="gallery" data-r style={{background:c.bg,padding:'clamp(4rem,8vh,7rem) 2rem'}}>
        <div style={{maxWidth:'1200px',margin:'0 auto'}}>
          <div style={{marginBottom:'2.5rem',...reveal('gallery')}}>
            <div style={{width:'32px',height:'2px',background:c.pink,marginBottom:'1.2rem'}}/>
            <h2 style={{fontSize:'clamp(28px,4.5vw,48px)',fontWeight:600,marginBottom:'.4rem'}}>Gallery</h2>
            <p style={{fontSize:'15px',color:c.muted,fontWeight:300}}>Moments from the pitch</p>
          </div>

          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gridAutoRows:'200px',gap:'5px'}}>
            {imgs.map((img,i)=>(
              <div key={i} style={{
                gridColumn:img.col,gridRow:img.row,overflow:'hidden',borderRadius:'2px',
                ...reveal('gallery',.05+i*.04),
              }}>
                <img src={img.src} alt="" loading="lazy"
                  style={{
                    width:'100%',height:'100%',objectFit:'cover',objectPosition:img.pos,
                    filter:'brightness(.9)',transition:`transform .7s ${ease}, filter .4s ${ease}`,cursor:'pointer',
                  }}
                  onMouseEnter={e=>{e.target.style.transform='scale(1.04)';e.target.style.filter='brightness(1.05)';}}
                  onMouseLeave={e=>{e.target.style.transform='scale(1)';e.target.style.filter='brightness(.9)';}}
                />
              </div>
            ))}
          </div>
  ...
  // ═══════════════════════════════════════════
  const Membership = () => (
    <section id="membership" data-r style={{background:c.card,padding:'clamp(4rem,8vh,7rem) 2rem',borderTop:'1px solid rgba(255,255,255,.04)'}}>
      <div style={{maxWidth:'1100px',margin:'0 auto'}}>
        <div style={{marginBottom:'2.5rem',textAlign:'center',...reveal('membership')}}>
          ...
          <h2 style={{fontSize:'clamp(28px,4.5vw,48px)',fontWeight:600,marginBottom:'.4rem'}}>Membership</h2>
          <p style={{fontSize:'15px',color:c.muted,fontWeight:300}}>Choose your commitment level</p>
        </div>

        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1.5rem'}}>
          {[
            {name:'Drop-In',price:'$20',per:'per session',feats:['Single match','No commitment','Walk-on flexibility'],feat:false},
            {name:'Player',price:'$120',per:'per month',feats:['Unlimited games','Priority booking','Player community','Kit discounts'],feat:true},
            {name:'Captain',price:'$200',per:'per month',feats:['Team slots','Coach access','Premium stats','Exclusive events'],feat:false},
          ].map((p,i)=>(
            <div key={i} style={{
            ...
            >
              {p.feat && <div style={{
                ...
                letterSpacing:'1.5px',textTransform:'uppercase',animation:'pulse 3s ease-in-out infinite',
              }}>Most Popular</div>}
              ...
              <div style={{borderTop:'1px solid rgba(255,255,255,.06)',paddingTop:'1.2rem',marginBottom:'1.8rem'}}>
                {p.feats.map((f,fi)=>(
                  <div key={fi} style={{display:'flex',alignItems:'center',gap:'.6rem',marginBottom:'.6rem',fontSize:'13px',color:'rgba(240,239,239,.75)'}}>
                    <div style={{width:'5px',height:'5px',background:c.pink,borderRadius:'50%',flexShrink:0}}/>
                    {f}
                  </div>
                ))}
              </div>
              <button style={{
                width:'100%',padding:'12px',background: p.feat ? c.pink : 'transparent',
                color: p.feat ? '#fff' : c.white,
                border: p.feat ? 'none' : '1px solid rgba(255,255,255,.15)',
  ...
  // ═══════════════════════════════════════════
  //  CTA — big parallax moment
  // ═══════════════════════════════════════════
  const CTA = () => (
    <Parallax src={BASE+'images/DSC08800.jpg'} h="55vh" pos="center 30%" overlay={.55}>
      <div style={{maxWidth:'600px'}}>
        <h2 style={{fontSize:'clamp(32px,6vw,72px)',fontWeight:900,lineHeight:1.05,letterSpacing:'-.02em',marginBottom:'1.2rem'}}>
          Ready to<br/>Play?
        </h2>
        <p style={{fontSize:'clamp(15px,1.8vw,19px)',color:'rgba(240,239,239,.85)',fontWeight:300,lineHeight:1.7,marginBottom:'2rem'}}>
          The most competitive pickup soccer community in Jersey.
        </p>
        <a href="https://opensports.net/21fc" target="_blank" rel="noopener noreferrer"
          style={{padding:'16px 48px',background:c.pink,color:'#fff',fontSize:'13px',fontWeight:700,
  ...
  // ═══════════════════════════════════════════
  const Contact = () => (
    <section id="join" data-r style={{background:c.bg,padding:'clamp(4rem,8vh,7rem) 2rem',borderTop:'1px solid rgba(255,255,255,.04)'}}>
      <div style={{maxWidth:'900px',margin:'0 auto'}}>
        <div style={{marginBottom:'2.5rem',...reveal('join')}}>
          <div style={{width:'32px',height:'2px',background:c.pink,marginBottom:'1.2rem'}}/>
          <h2 style={{fontSize:'clamp(28px,4.5vw,48px)',fontWeight:600,marginBottom:'.4rem'}}>Get in Touch</h2>
          ...
          <form style={{display:'flex',flexDirection:'column',gap:'1.2rem'}} onSubmit={e=>e.preventDefault()}>
            {['Your name','your@email.com'].map((ph,i)=>(
              <input key={i} type={i===1?'email':'text'} placeholder={ph}
                style={{
                  background:'transparent',border:'none',borderBottom:'1px solid rgba(255,255,255,.1)',
            ...
            <button type="submit" style={{
              padding:'12px 24px',background:c.pink,color:'#fff',border:'none',fontSize:'12px',
  ...
  // ═══════════════════════════════════════════
  const Location = () => (
    <section id="location" data-r style={{background:'#060610',padding:'clamp(4rem,8vh,7rem) 2rem',borderTop:'1px solid rgba(255,255,255,.04)'}}>
      <div style={{maxWidth:'1100px',margin:'0 auto'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1.6fr',gap:'clamp(2rem,4vw,3.5rem)',alignItems:'start'}}>
          <div style={reveal('location')}>
            <div style={{width:'32px',height:'2px',background:c.pink,marginBottom:'1.2rem'}}/>
            <h2 style={{fontSize:'clamp(28px,4.5vw,48px)',fontWeight:600,marginBottom:'.4rem'}}>Find Us</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'1.5rem'}}>
              {[
                ...
                <div key={i} style={{paddingBottom:'1.2rem',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
                  ...
                  {x.link ? (
                    <a href={x.link} target="_blank" rel="noopener noreferrer" style={{
                      fontSize:'15px',fontWeight:500,color:c.white,textDecoration:'none',
                      borderBottom:'1px solid rgba(237,17,113,.3)',paddingBottom:'1px',
                      transition:`border-color .3s ${ease}`,
                    }}
          ...
          <div style={{borderRadius:'2px',overflow:'hidden',border:'1px solid rgba(255,255,255,.05)',height:'400px',...reveal('location',.15)}}>
      ...
      <div style={{maxWidth:'1100px',margin:'0 auto'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem',flexWrap:'wrap',gap:'1.5rem'}}>
          <div style={{display:'flex',alignItems:'center',gap:'.6rem'}}>
            <img src={BASE+"images/logo-court-baller-white.png"} alt="21FC" style={{height:'28px',width:'auto'}} loading="lazy"/>
            <span style={{fontWeight:700,fontSize:'16px'}}>21FC</span>
          </div>
  ...
  return (
    <>
      ...
      <ImageBand images={[
        ...
        {src:BASE+'images/DSC08674.jpg',pos:'center 40%'},
        {src:BASE+'images/DSC08737.jpg',pos:'center 35%'},
      ]} />
      <Schedule />
      <Parallax src={BASE+'images/DSC08619.jpg'} h="35vh" pos="center 35%" overlay={.5} />
      <Gallery />
      <Membership />
      <CTA />
      <Contact />
      <Parallax src={BASE+'images/DSC08674.jpg'} h="30vh" pos="center 40%" overlay={.55} />
      <Location />
      <Footer />
    </>
  );
